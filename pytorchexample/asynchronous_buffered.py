            ...
            while global_updates_done < max(1, num_rounds):
                ...
                for reply in replies:
                    ...
                    tier_label = _id_to_tier.get(
                        int(client_metrics.get("straggler_tier_id", -1)), "")
                    ...
                    if len(buffered_replies) >= self.async_buffer_size:
                        ...
                        for _br, _bt, _be in buffered_replies:
                            ...
                            _br_content = _br.content.get(
                                "metrics") if _br.has_content() else None
                            if _br_content is not None:
                                _tid = int(_br_content.get(
                                    "straggler_tier_id", -1))
                        ...
                        if _buf_weights:
                            _buf_comp_log["buffer_mean_staleness_weight"] = sum(
                                _buf_weights) / len(_buf_weights)
                            ...
                            if evaluate_fn is not None and global_updates_done % evaluation_interval == 0:
                                ...
                                if eval_res is not None:
                                    ...
                                    if accuracy is not None:
                                        ...
                                        wandb.log(
                                            {"peak_top1_test_accuracy": peak_accuracy}, step=global_updates_done)
